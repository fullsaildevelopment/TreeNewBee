...
bool AHeavyMelee_Ally::OnParry(const struct FDamageInfo* const _damageInfo, const class UPawnStatsComponent* const _damageCauserPawnStats)
{

	bool accept = false;
	if (CharacterType != ECharacterType::Ebony_Heavy_BA)
		return false;
...
int AHeavyMelee_Ally::GetMeleeComboSel(bool _bIsMoving) const
{
	//return 0;
	bool IsQueensGuard = CharacterType == ECharacterType::Ebony_Heavy_BA;
	if (_bIsMoving)
...
FName AHeavyMelee_Ally::GetParrySectionName(const struct FDamageInfo* const _damageInfo) const
{
...
bool AHeavyMelee_Ally::ShouldPlayHitAnimation() const
{
